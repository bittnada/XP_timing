...
class Timer(object):
    ...
    def __call__(self, params, placedb):
        ...
        """
        tt = time.time()
        self.raw_timer = placedb.initialize_timing_db(params, timing_cpp)
        if self.raw_timer is None:
            self.read(params)
    ...
    def report_timing_paths(self, n, split=True):
        """Exact traversed pins/cells, slack and arrival in native time units.

        split=True selects MAX/setup; False selects MIN/hold. This does not
        expand path nets into their fanout cells. Call after update_timing().
        """
        if not hasattr(timing_cpp, "report_timing_paths"):
            raise RuntimeError("Rebuild and install timing_cpp for critical-path extraction")
        return timing_cpp.report_timing_paths(self.raw_timer, n, split)
