...
class PlaceDB (TimingCacheMixin):
    """
    @brief placement database
    """
    def __init__(self):
    ...
    def read(self, params):
        ...
        if MakeDBAdapter.enabled(params):
            self.rawdb, pydb = MakeDBAdapter.read(params)
            self.initialize_from_rawdb(params, pydb)
            for field in ('num_movable_std_cell', 'num_movable_macro', 'num_fixed_std_cell',
                          'num_fixed_macro', 'num_fake_macro', 'num_blockage'):
                setattr(self, field, getattr(pydb, field))
            return
        self.rawdb = place_io.PlaceIOFunction.read(params)
        self.initialize_from_rawdb(params)

    def initialize_from_rawdb(self, params, pydb=None):
        """
    ...
    def __call__(self, params):
        ...
        self.read(params)
        PlacementState.read_initial(params, self)
        if getattr(self.rawdb, 'is_makedb', False):
            import MakeDBAdapter
            MakeDBAdapter.finish_initial(self, params)
        self.initialize(params)
        self.prepare_timing_db(params)
...
""" % (
    ...
    def write(self, params, filename, sol_file_format=None):
        ...
        if getattr(self.rawdb, 'is_makedb', False) and sol_file_format != place_io.SolutionFileFormat.BOOKSHELF:
            self.rawdb.write_def(filename, node_x, node_y)
        elif ((getattr(self.rawdb, 'is_makedb', False) or
               (not params.legalize_flag and not params.detailed_place_flag))
    ...
    def write_pl(self, params, pl_file, node_x, node_y):
        ...
        for i, node_id in enumerate(fixed_node_indices):
            content += "\n%s %g %g : %s /FIXED" % (
                    ...
                    (PlacementState.ORIENTS[int(self.rawdb.node(node_id).orient())]
                     if getattr(self.rawdb, 'is_makedb', False) else "N")
                    )
    ...
    def apply(self, params, node_x, node_y):
        ...
        # update raw database
        if getattr(self.rawdb, 'is_makedb', False):
            self.rawdb.apply(params, node_x, node_y)
        else:
            place_io.PlaceIOFunction.apply(self.rawdb, node_x, node_y)
        if (getattr(self.rawdb, 'is_makedb', False) or
                getattr(params, 'read_orient', '') or getattr(params, 'write_orient', '')):
            self.node_orient = np.asarray(PlacementState.final_orientations(self), dtype='S7')
