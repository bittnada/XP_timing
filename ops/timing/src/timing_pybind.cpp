...
std::vector<std::vector<int> > _report_timing(
  ...
  for (auto& path : paths) {
    ...
    for (auto& point : path) {
      ...
      auto mapped = net_name2id_map.find(name);
      if (mapped == net_name2id_map.end()) continue;
      int net_id = mapped->second;
      if (net_id != prev)
...
// Return exact traversed pins/gates, not all sinks of each path net.
// Slack and arrival times retain OpenTimer's native time unit.
pybind11::list _report_timing_paths(ot::Timer& timer, int n, bool split) {
  if (n <= 0) throw pybind11::value_error("path count must be positive");
  pybind11::list result;
  for (const auto& path : timer.report_timing(n, static_cast<ot::Split>(split))) {
    pybind11::dict record;
    record["slack"] = path.slack;
    record["split"] = split ? "MAX" : "MIN";
    pybind11::list points;
    for (const auto& point : path) {
      pybind11::dict item;
      item["pin"] = point.pin.name();
      const auto* gate = point.pin.gate();
      item["cell"] = gate ? pybind11::cast(gate->name()) : pybind11::none();
      item["transition"] = point.transition == ot::RISE ? "rise" : "fall";
      item["arrival"] = point.at;
      points.append(item);
    }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.attr("timing_cache_schema") = 2;
  m.def("compile_timing_model", &dreamplace_timing_cache::compile,
        "Parse LIB/Verilog/SDC once and save a versioned binary model",
        pybind11::call_guard<pybind11::gil_scoped_release>());
  m.def("load_timing_model", &dreamplace_timing_cache::load,
        "Restore OpenTimer from a parsed binary model (no text parsing)",
        pybind11::call_guard<pybind11::gil_scoped_release>());
  pybind11::class_< // This binding converts a unique pointer.
    ...
    >(m, "Timer")
      ...
      .def("dump_celllib_file", [](ot::Timer& timer, const std::string& out, bool late) {
        std::ofstream f(out);
        if(!f) throw std::runtime_error("Cannot write celllib dump: " + out);
        timer.dump_celllib(f, late ? ot::MAX : ot::MIN);
      }, pybind11::arg("path"), pybind11::arg("late") = true)
      ...
      .def("report_slew", [](ot::Timer& timer, const std::string& pin, bool split, bool tran) {
        return timer.report_slew(pin, static_cast<ot::Split>(split), static_cast<ot::Tran>(tran)).value_or(std::nanf(""));
      })
  ...
  m.def("report_timing_paths", &DREAMPLACE_NAMESPACE::_report_timing_paths,
        pybind11::arg("timer"), pybind11::arg("n"), pybind11::arg("split") = true,
        "Report exact path pins/cells and native-unit slack (MAX by default)");
