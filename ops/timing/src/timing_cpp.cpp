...
int timingCppLauncher(
  ...
  // MakeDB preserves DEF pin order, which need not put the driver first.
  // Validate outside OpenMP so mismatches raise Python errors, not aborts.
  std::vector<int> roots(num_nets, -1);
  for (int i = 0; i < num_nets; ++i) {
    if (net_names[i].empty()) continue; // explicitly excluded placement-only net
    auto nit = timer.nets().find(net_names[i]);
    if (nit == timer.nets().end() || !nit->second.root())
      throw std::runtime_error("Physical net missing or undriven in timer: " + net_names[i]);
    for (int j = netpin_start[i]; j < netpin_start[i + 1]; ++j) {
      const int p = flat_netpin[j];
      auto pit = timer.pins().find(pin_names[p]);
      if (pit == timer.pins().end() || pit->second.net() != &nit->second)
        throw std::runtime_error("Physical/timing pin connectivity mismatch: " + pin_names[p]);
      if (&pit->second == nit->second.root()) roots[i] = p;
    }
    if (roots[i] < 0)
      throw std::runtime_error("Timing driver missing in physical net: " + net_names[i]);
  }
