...
struct NetWeighting<T, NetWeightingScheme::ADAMS> {
  DEFINE_APPLY_SCHEME {
    ...
    size_t num_nets = 0;
    for (const auto& item : net_name2id_map)
      num_nets = std::max(num_nets, size_t(item.second + 1));
    std::vector<bool> net_critical_flag(num_nets, 0);
    std::vector<bool> mapped_flag(num_nets, 0);
    for (const auto& item : net_name2id_map) mapped_flag[item.second] = 1;
...
#pragma omp parallel for num_threads(num_threads)
    for (size_t i = 0; i < num_nets; ++i) {
      // Placement-only nets must retain their explicit user weight.
      if (!mapped_flag[i]) continue;
