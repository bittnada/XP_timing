...
def place(params):
    ...
    PlacementState.validate_paths(params)
    ...
    if getattr(params, "timing_clustering_flag", 0):
        TimingCluster.run(params, placedb, timer=timer)
    ...
    PlacementState.write_final(params, placedb)
...
if __name__ == "__main__":
    ...
    params.printWelcome()
    args = PlacementState.parse_cli()
    # CLI overrides the corresponding optional JSON settings.
    params.load(args.config)
    for action in ('read', 'write'):
        for field in PlacementState.FIELDS:
            key = action + '_' + field
