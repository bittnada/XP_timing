...
class NonLinearPlace(BasicPlace.BasicPlace):
    ...
    def __init__(self, params, placedb, timer):
        ...
        # Native PlaceIO supplies canonical-N movable pin offsets unless a
        # checkpoint transformed them; MakeDB already applies orientation.
        self._metric_pin_orientations = np.array([
            PlacementState.text(o) for o in placedb.node_orient[:placedb.num_movable_nodes]])
        if not getattr(placedb.rawdb, 'is_makedb', False) and not getattr(params, 'read_orient', ''):
            self._metric_pin_orientations[:] = 'N'
    ...
    def __call__(self, params, placedb):
        ...
        # Evaluate every final placement. write_orient controls persistence only.
        import PlacementState
        import FinalPlacementMetrics
        placedb.node_orient = np.asarray(PlacementState.final_orientations(placedb), dtype='S7')
        FinalPlacementMetrics.synchronize_pin_geometry(self, placedb)
        FinalPlacementMetrics.evaluate(self, params, placedb, iteration)
        # plot placement
        if params.plot_flag:
            self.plot(params, placedb, iteration, cur_pos)
        return all_metrics
